    ...
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(Commands.literal("invincible")
                        ...
                        .then(Commands.argument("value", IntegerArgumentType.integer())
                                .executes((context) -> {
                                    if(context.getSource().getPlayer() != null){
                                        InvinciblePlayerCapabilityProvider.get(context.getSource().getPlayer()).setPhase(IntegerArgumentType.getInteger(context, "value"));
                                    }
                                    return 0;
                                })
                        )
                ...
                .then(Commands.literal("resetPhase").requires((commandSourceStack) -> commandSourceStack.hasPermission(2))
                        .executes((context) -> {
                            if(context.getSource().getPlayer() != null){
                                InvinciblePlayerCapabilityProvider.get(context.getSource().getPlayer()).resetPhase();
                            }
                            return 0;
                        })
                )
